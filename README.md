<h1 align="center">👋 Merhaba, ben Ali</h1>

<p align="center">
  Bilgisayar mühendisliği öğrencisiyim. Web, mobil, yapay zeka ve sistem programlama gibi alanlarda projeler geliştirdim ve geliştirmeye devam ediyorum.
</p>

---

## 🚀 GitHub Projelerim

| Proje | Açıklama |
|-------|----------|
| 🔍 [**Article-Anonymization-System**](https://github.com/aliclsr/Article-Anonymization-System) | Akademik PDF makaleleri otomatik olarak anonimleştiren FastAPI tabanlı sistem |
| 🗓️ [**Eventify**](https://github.com/aliclsr/Eventify) | Akıllı etkinlik planlama platformu (FastAPI + HTML) |
| 🧠 [**Field-of-Interest-Tracker**](https://github.com/aliclsr/Field-of-Interest-Tracker) | Kullanıcı ilgi alanlarına göre öneriler sunan Python tabanlı uygulama |
| 🏥 [**Hospital-System**](https://github.com/aliclsr/Hospital-System) | C# ile masaüstü hasta kayıt ve randevu yönetimi sistemi |
| 🧵 [**Thread-Processing**](https://github.com/aliclsr/Thread-Processing) | Çok iş parçacıklı C# uygulaması – stok ve sipariş senkronizasyonu |
| 🎫 [**Ticket-Reservation-System**](https://github.com/aliclsr/Ticket-Reservation-System) | Java tabanlı bilet rezervasyon sistemi |
| 🗺️ [**Treasure-Hunter-Game**](https://github.com/aliclsr/Treasure-Hunter-Game) | Harita üzerinde rastgele hazine arama oyunu |
| 🎮 [**WordBattle**](https://github.com/aliclsr/Wordbattle) | Flutter + FastAPI ile geliştirilen 2 kişilik çevrimiçi kelime oyunu (Mobile) |
| 🧱 [**Breakout-Game**](https://github.com/aliclsr/Breakout-Game) | Tuğla kırma oyunu – masaüstü oyun projesi |
| 🏞️ [**TurizmWeb**](https://github.com/aliclsr/TurizmWeb) | Turizm sektöründe hizmet veren bir şirketin iletişim ve tanıtım ihtiyaçlarını karşılayan modern bir web sitesi|
| 🐨 [**Image-Processing-to-Recognize-Animals**](https://github.com/aliclsr/Image-Processing-to-Recognize-Animals) | Yüklenen hayvan görsellerinden 100'e yakın hayvanı tanıyabilen bir görüntü işleme modeli |

---

## 🛠️ Teknolojiler

- Python (FastAPI, OpenCV, PyTorch, Torchvision, Timm, Matplotlib)
- Google Colab
- Flutter & Dart
- C# (.NET 4.8, Windows Forms)
- SQL (SQLite, MSSQL)
- Git & GitHub
- HTML, TailwindCSS
  
---

## 📫 Bana Ulaşın

- GitHub: [github.com/aliclsr](https://github.com/aliclsr)
- Mail: **aliefkan2003@gmail.com**
